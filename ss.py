...
def check_color(frame, color):
    ...
    if len(cnt_color) > 0:
        ...
        color_flag = 0
    return frame,rect_red[0][0],rect_red[1][0]

def addrouter(routeList, routeNodeIndex, x, width):
    global route_flag
    ...
    if route_flag == 1 :
        if x<minY :
            ...
            route_flag = 2
        routeList.insert(routeNodeIndex+1, [X, y_new, Z, 1, 0, 0, 0])
    # 前后调整
    if route_flag == 2 :
        if width<minwidth :
                x_new = x-0.2
                ...
                x_new = x+0.2
        routeList.insert(routeNodeIndex+1 , [x_new, Y, Z, 1, 0, 0, 0])
...
def detect():
    while (True):
        frame = cap.read()
        width, x, y, frame =check_color(frame, "green")
        #ov.show(check_color(frame, "green"))
...
def addCircleRoute(routeList, routeNodeIndex, radius, direction):
    radianList = [
        10 / 6 * pi, 11 / 6 * pi, 2 * pi, 1 / 6 * pi, 2 / 6 * pi, 3 / 6 * pi,
        4 / 6 * pi, 5 / 6 * pi, pi, 7 / 6 * pi, 8 / 6 * pi
    ]
    x = float(routeList[routeNodeIndex][0])
    y = float(routeList[routeNodeIndex][1])
    z = float(routeList[routeNodeIndex][2])
    ...
    if (direction == 'clock'):
        for i in range(len(radianList)):
            x_new = round(x + radius * math.cos(radianList[i]), 2)
            y_new = round(y + radius * (1 + math.sin(radianList[i])), 2)
            routeList.insert(routeNodeIndex + 1, [x_new, y_new, z, 1, 0, 0, 0])
    # 生成逆时针路径
    elif (direction == 'cntclock'):
        for i in range(len(radianList)):
            x_new = round(
                x + radius * math.cos(radianList[len(radianList) - 1 - i]), 2)
            y_new = round(
                y + radius *
                (1 + math.sin(radianList[len(radianList) - 1 - i])), 2)
            routeList.insert(routeNodeIndex + 1, [x_new, y_new, z, 1, 0, 0, 0])
    # print(circleList)
...
def Router(name):
    ...
    if routeNodeIndex < routeNodeNum and routeStartFlag == True:
        ...
        if (routeNodeIndex==2 and route_flag == 1):
            frame = cap.read()
            frame, x, width=check_color(frame, "green")
            addrouter(routeList, routeNodeIndex, x, width)
            print("\nafter add: " + str(routeList[routeNodeIndex+1]))
